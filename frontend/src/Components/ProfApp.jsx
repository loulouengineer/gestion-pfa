...
import { Calendar, MessageCircle, Bell, ClipboardList, LogOut, ChevronRight } from "lucide-react";
import { chatApi } from "../api/api";
import { NotificationCenter } from "./NotificationPopup";
...
function AppBanner() {
  return (
    <div style={{ position: "fixed", top: 0, right: 0, padding: "8px 20px", zIndex: 100, display: "flex", alignItems: "center", gap: 8 }}>
      <span style={{ fontSize: 11, color: "var(--text-4)", fontWeight: 600, letterSpacing: "0.06em", textTransform: "uppercase" }}>Gestion PFA</span>
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "var(--bg)" }}>
      <aside style={{
        ...
        {/* Prof avatar + Bell — same layout as admin */}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, flex: 1, minWidth: 0 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 10, color: "#475569" }}>
                {prof.departement || "Professeur"}
              </div>
            </div>
          </div>
          {/* Notification bell — same as admin */}
          <NotificationCenter onNavigate={(page) => {
            if (page === "notifs") setActive("notifications");
            else handleSelect(page);
          }} />
        </div>
        ...
        <nav style={{ padding: "14px 10px", flex: 1, overflow: "auto" }}>
          ...
          <div style={{ marginTop: 16 }}>
            <NavItem
              item={{ id: "notifications", label: "Notifications", sub: "Alertes & rappels", icon: Bell }}
              active={active}
              onSelect={handleSelect}
            />
          </div>
      ...
      <main style={{ flex: 1, overflow: "auto", padding: "40px 48px" }}>
        <AppBanner />
        <div style={{
          position: "fixed", top: 0, right: 0, width: 800, height: 600,
          pointerEvents: "none", zIndex: 0,
          background: "radial-gradient(ellipse at 85% 0%, rgba(59,130,246,0.10) 0%, rgba(124,58,237,0.05) 40%, transparent 70%)",
        }} />
        <div style={{
          position: "fixed", bottom: 0, left: 240, width: 600, height: 400,
          pointerEvents: "none", zIndex: 0,
          background: "radial-gradient(ellipse at 20% 100%, rgba(16,185,129,0.06) 0%, transparent 60%)",
        }} />
        <div style={{ position: "relative", zIndex: 1, maxWidth: 1000, margin: "0 auto" }}>
          ...
          {active === "notifications"  && <ProfNotifications  prof={prof} />}
        </div>
      </main>
    </div>
  );
