...
import tn.enicarthage.projetspring.service.SoutenanceService;
...
public class CreneauController {
    ...
    public ResponseEntity<List<Map<String, Object>>> getTous() {
        return ResponseEntity.ok(creneauRepository.findAll().stream()
    ...
    public ResponseEntity<?> modifier(@PathVariable Long id, @RequestBody Map<String, Object> body) {
        ...
        if (creneau.getStatut() == StatutCreneau.OCCUPE) {
            return ResponseEntity.badRequest().body(Map.of(
                    "erreur", "Ce créneau est occupé. Annulez la soutenance pour le modifier."));
        }

        if (body.containsKey("date")) creneau.setDate(LocalDate.parse((String) body.get("date")));
        if (body.containsKey("heureDebut")) creneau.setHeureDebut(LocalTime.parse((String) body.get("heureDebut")));
        if (body.containsKey("dureeMinutes")) creneau.setDureeMinutes(Integer.parseInt(body.get("dureeMinutes").toString()));
        if (body.containsKey("salle")) creneau.setSalle((String) body.get("salle"));
    ...
    private Map<String, Object> toMap(Creneau c) {
        ...
        map.put("jury", jury.stream().map(p -> Map.of("id", p.getId(), "nom", p.getNom()))
                .collect(Collectors.toList()));
        return map;
    }
}
